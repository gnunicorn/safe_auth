...
/// TODO: docs
pub fn make_to_uri<T>(app_id: String, action: String, data: &T, app_id_is_receiver: bool) -> Result<String, ProtocolError>
    where T: Encodable
{
...
mod tests {
    ...
    fn simple_back_and_forth() {
        ...
        let uri = make_to_uri("net.maidsafe.example.test".to_string(), "auth".to_string(), &data, false).unwrap();
        println!("{:}", uri);
    ...
    fn sending_back() {
        let data = (1u64..8).collect::<Vec<_>>();
        let uri = make_to_uri("net.maidsafe.example.test".to_string(), "register".to_string(), &data, true).unwrap();
        println!("{:}", uri);
        let (app_id, action, payload) : (String, String, Vec<u64>) = parse_from_uri(uri).unwrap();
    ...
    fn complicated_back_and_forth() {
        ...
        let uri = make_to_uri("net:maidsafe".to_string(), "call-to-action".to_string(), &data, false).unwrap();
        println!("{:}", uri);
